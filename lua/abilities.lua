...
function wesnoth.wml_actions.quick_on_my_feet(cfg)
    ...
    elseif mode == "moved" then
        ...
        if not unit or not M.include(unit.abilities, "quick_on_my_feet") then
            wesnoth.log("warning", "quick_on_my_feet: nem található egység ezzel az ID-vel és képességgel: " .. tostring(unit_id))
            return
        end
